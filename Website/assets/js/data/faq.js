export const FAQ = [
  ["What is Capsi?", "Capsi is a lightweight Windows utility for sending messages and files directly between computers on a connected local network."],
  ["Does Capsi require the internet?", "No internet connection is required for local device-to-device communication."],
  ["Do I need an account?", "No. There is no signup or login."],
  ["How do devices find each other?", "Capsi discovers other Capsi devices on the connected local network."],
  ["Can I send files?", "Yes. You can offer a file to an accepted device, and the other side can accept or decline the transfer."],
  ["Is my data stored in the cloud?", "Capsi does not use a Capsi cloud service for local conversations or file transfers."],
  ["Does Capsi work on macOS or Linux?", "The current release is for Windows. Other platforms may be supported later."],
  ["Is Capsi free?", "Capsi is free to download and use."]
];